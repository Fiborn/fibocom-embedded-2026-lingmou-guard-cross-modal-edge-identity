...
class LFWDataset(datasets.ImageFolder):
    ...
    def get_lfw_paths(self,lfw_dir,file_ext="bmp"):
        ...
        #for pair in pairs:
            ...
            if len(pair) == 3:
                ...
                path0 = os.path.join(lfw_dir, 'fake_fenzu', pair[1], pair[2]+'.'+file_ext)
                path1 = os.path.join(lfw_dir, 'real_fenzu', pair[3], pair[4]+'.'+file_ext)
                # path0 = os.path.join(lfw_dir,  pair[1], pair[2]+'.'+file_ext)
                # path1 = os.path.join(lfw_dir,  pair[3], pair[4]+'.'+file_ext)
                if int(pair[0]) == 0:
                    issame = True
                else:
                    issame = False
