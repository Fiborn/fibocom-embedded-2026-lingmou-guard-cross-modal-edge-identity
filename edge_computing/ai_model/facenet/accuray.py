...
def testImage():
    ...
    for per_image in test_image_list:
        ...
        image = Image.open(test_per_image_path)
        per_image_encoding = model.detect_image111(image)
    ...
    # print("识别完成！")
    a=len(false_right_reco_name_list)
    ...
    print("ceshi人脸图片数量：%s,检测到人脸图片数量：%s ,wei检测到人脸图片数量：%s,匹配人脸数量:%s,wei匹配到人脸图片:%s , 正确匹配人脸图片数量：%s,wei正确匹配人脸图片数量：%s" % (test_date_num,decate_count, undecate_count, reco_count, unsuccess_reco_count, right_reco_count,a))
